...
export default class TextMarquee extends PureComponent {
  ...
  animateScroll = () => {
    const {
      duration, marqueeDelay, loop, useNativeDriver, repeatSpacer, easing, children,
    } = this.props;
    this.setTimeout(() => {
  ...
  animateBounce = () => {
    const {
      duration, marqueeDelay, loop, useNativeDriver, easing, children,
    } = this.props;
    this.setTimeout(() => {
      Animated.sequence([
        Animated.timing(this.animatedValue, {
  ...
  render() {
    const {
      style, children, repeatSpacer, scroll, ...props
    } = this.props;
